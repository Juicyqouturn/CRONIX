...
class tiktok:
    ...
    def update_title(self):
        ...
        ctypes.windll.kernel32.SetConsoleTitleW(
            f"TikTok Username Checker | Available: {self.available} | Unavailable: {self.unavailable} | Checked: {(self.available + self.unavailable)} | Remaining: {remaining} | Developed by @juicyqouturn on instagram"
        )
    ...
    def print_console(self, status, arg, color = Fore.MAGENTA):
    ...
    def main(self):
        os.system(clear)
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW("TikTok Username Checker | Developed by @juicyqouturn on instagram")
        print(Fore.MAGENTA + ascii_text)
        self.load_usernames()
        threads = int(input(f"       {Fore.WHITE}[{Fore.GREEN}Console{Fore.WHITE}] Threads: "))
        print()
